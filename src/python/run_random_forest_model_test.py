...
output_root     = "/Volumes/T7 Shield/toxin_dataset/toxins_and_neuro/classifier_results/test_random_forest_csv_results"
...
for hom in homology_training_list:
    ...
    output_csv = os.path.join(
        output_root,
        f"random_forest_test_results_{hom}_hom_dup.csv"
    )
    ...
    if not X_test_list:
        print(f"  ⚠️  No test embeddings found for homology {hom}. Skipping.")
        continue
    ...
    feature_steps = range(5, 305, 10)
    results = []

    for num_feat in feature_steps:
        model_path = os.path.join(model_dir, f"randomforest_hom{hom}_{num_feat}.joblib")
        ...
        print(f"  • {num_feat} features → acc {acc:.2f}% | bal acc {bacc:.2f}%")

        # Print confusion matrix and classification report at 100 features
        if num_feat == 100:
            print("\n=== Confusion Matrix and Classification Report (100 features) ===")
