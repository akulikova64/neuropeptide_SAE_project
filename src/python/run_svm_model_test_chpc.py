#!/usr/bin/env python3
"""
Test RBF-SVM models on their excluded folds (mirrors your training layout).

Writes, for each fold k:
  ./output_csv_files/excluded_fold_k/svm_rbf_test_results_test_fold_k.csv

Each CSV has rows for every saved model size (num_features).
"""
...
# Ensure these imports are present so joblib can unpickle the Pipeline
...
# ─── Paths (aligned with your SVM training script) ─────────────────────────
...
# Where SVM models were saved by the training script
models_root_template = "./output_models/svm_model/excluded_fold_{fold}/"

# Where to write per-fold test CSVs
output_root_template = "./output_csv_files/svm_model/excluded_fold_{fold}/"


# ─── Helpers ───────────────────────────────────────────────────────────────
def load_rank_indices(path_csv: str) -> list[int]:
    """Return list of feature indices sorted by ascending rank."""
    df = pd.read_csv(path_csv).sort_values("rank")
    return df["feature"].astype(int).tolist()
...
def load_labels_and_embeddings():
    ...
    if missing:
        print(f"⚠️  Missing embeddings for entries: {len(missing)}")
...
def main():
    ...
    for test_fold in test_folds:
        ...
        # — Locate saved SVM models for this fold
        models_dir = models_root_template.format(fold=test_fold)
        if not os.path.isdir(models_dir):
            print(f"  ⚠️  Models dir missing: {models_dir}; skipping.")
            continue

        # Models are named: svm_rbf_final_features_{N}.joblib
        model_files = sorted(
            f for f in os.listdir(models_dir)
            if re.match(r"svm_rbf_final_features_(\d+)\.joblib$", f)
        )
        if not model_files:
            print(f"  ⚠️  No SVM model files found in {models_dir}; skipping.")
            continue
        ...
        out_csv = os.path.join(
            out_dir,
            f"svm_rbf_test_results_test_fold_{test_fold}.csv"
        )
        ...
        for fn in model_files:
            m = re.match(r"svm_rbf_final_features_(\d+)\.joblib$", fn)
            ...
            # Load Pipeline(Sclaer+SVC) and predict
