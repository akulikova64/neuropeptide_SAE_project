#!/usr/bin/env python3
"""
Run 3 models (LogReg, CRF, SVM) on novo_alldata_l18 embeddings and
write a single CSV with per-residue probabilities from each model plus
their mean and per-peptide normalized probabilities.

FASTA constraint:
  Only entries present in ../../data/novo_alldata_l18_no_TMRs_highprob.fasta
  are used.
...
# >>> EDIT THIS if your embedding directory differs <<<
#EMB_DIR    = "/Volumes/T7 Shield/layer_33_highest_scoring_novo_embeddings"
EMB_DIR    = "/Volumes/T7 Shield/layer_33_50_60_score_novo_embeddings"

# Only use sequences present in this FASTA:
#FASTA_PATH = "../../data/novo_alldata_l18_no_TMRs_highprob.fasta"
FASTA_PATH = "../../data/novo_alldata_l18_no_TMRs_50_to_60.fasta"

# the ranking file stays the same
...
#OUT_CSV = os.path.join(OUT_DIR,"novo_alldata_l18_no_TMRs_highprob_logreg_crf_svm_probs.csv")
OUT_CSV = os.path.join(OUT_DIR,"novo_alldata_l18_no_TMRs_50_60_logreg_crf_svm_probs.csv")
...
def try_load_svm_feature_indices_from_meta(model_path: str, expect_n: int) -> list[int] | None:
...
def main():
    ...
    # Filter embeddings to only those entries present in the FASTA
    ...
    print(f"✔ Loaded CRF model from {CRF_MODEL_PATH} (positive label = {crf_pos_label!r})")
    ...
    with open(OUT_CSV, "w", newline="") as fh:
        ...
        for idx, emb_fp in enumerate(emb_paths_filtered, 1):
            ...
            L, D = arr.shape
            if max(feats_logreg) >= D or max(feats_crf) >= D or max(svm_feats) >= D:
                print(f"⚠️  Feature index out of bounds for {entry} (D={D}); skipping.")
                continue
